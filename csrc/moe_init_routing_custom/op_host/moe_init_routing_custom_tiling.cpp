...
ge::graphStatus MoeInitRountingCustomTilingBase::GetWorkspaceSize()
{
    ...
    size_t coreSyncWorkspaceSize =
        moeInitRoutingCustomTilingData.get_coreNum() * SORT32_ALIGN_ELEMENT * NUM_TWO; // 多核同步需要的空间
...
void MoeInitRountingCustomTilingBase::Tiling4GatherOutCompute()
{
    ...
    if (quantMode_ == STATIC_QUANT) {
        colMultiple = SIZE_INT8 * NUM_TWO + SIZE_FP32 + SIZE_INT16 + inuptXDtypeSize_ * NUM_TWO;//根据quantv2修改
        rowMultiple = NUM_TWO;
    }
