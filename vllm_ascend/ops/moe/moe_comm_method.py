...
def setup_moe_comm_method(moe_config):
    ...
    _MoECommMethods[MoECommType.NAIVE_MULTICAST] = NaiveMulticastCommImpl(
        moe_config)
    _MoECommMethods[MoECommType.EP_ALLGATHER] = EPAllgatherCommImpl(moe_config)
...
class EPAllgatherCommImpl(MoECommMethod):
    """This implementation is the same as NativeAllGatherCommImpl,
    but uses NPU-specific ops for better performance.
    ...
    def _get_fused_moe_prepare_finalize(self):
        return FusedMoEPrepareAndFinalizeWithEPAllgather(self.moe_config)
